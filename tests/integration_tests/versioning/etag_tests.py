...
class TestETagEmission(SupersetTestCase):
    ...
    def test_chart_get_version_emits_etag(self) -> None:
        ...
        rv = self.client.get(
            f"/api/v1/chart/{chart.uuid}/versions/{version_uuid}/"
        )
        assert rv.status_code == 200
    ...
    def test_etag_absent_when_entity_has_no_version_rows(self) -> None:
        ...
        # Restore version rows for downstream tests by re-saving.
        rv = self.client.put(
            f"/api/v1/chart/{chart_id}",
            json={"slice_name": "etag-restore-versions"},
        )
        assert rv.status_code == 200
        # Reset name to keep fixture stable.
        rv = self.client.put(
            f"/api/v1/chart/{chart_id}",
            json={"slice_name": "Girls"},
        )
        assert rv.status_code == 200
        # Make sure re-creation wired up.
        assert (
