...
def _resolve_shadow_tables() -> tuple[list[sa.Table], list[sa.Table], sa.Table | None]:
    ...
    m2m_table = (
        metadata.tables.get("dashboard_slices_version") if metadata is not None else None
    )
...
def _delete_for_transactions(
    ...
    *tx_ids*. Returns total rowcount across all tables.
    ...
    for tbl in tables:
        result = conn.execute(
            sa.delete(tbl).where(tbl.c.transaction_id.in_(tx_ids))
        )
        total += result.rowcount or 0
    return total


def _prune_old_versions_impl(retention_days: int) -> dict[str, int]:
    ...
    with db.engine.begin() as conn:
        ...
        tx_rows = conn.execute(
            sa.delete(tx_table).where(tx_table.c.id.in_(tx_ids))
        ).rowcount or 0
