...
def set_version_etag(
    response: "Response", version_uuid: Optional[UUID]
) -> "Response":
    """Attach ``ETag: "<version_uuid>"`` to *response*.

    No-op when *version_uuid* is ``None`` (entity has no version rows yet).
    """
...
def set_version_etag_by_uuid(
    response: "Response", model_cls: type, entity_uuid: UUID
) -> "Response":
    ...
    entity_id = (
        ...
        .filter(model_cls.uuid == entity_uuid)
        .scalar()
    )
